...
[章目次](./chap8.md)
## 8.5. 符号付きグラフのニューラルネットワーク

現実世界では，正(ポジティブ)の関係性と負(ネガティブ)の関係性の両方を持つシステムに遭遇する場面がある． 例えば，SNSのユーザは，友だち（例：FacebookやSlashdot [^1] ），フォロワー（例：Twitter），信頼(例:Epinions [^2] )などのポジティブなエッジを持つだけでなく，敵・アンチ(例：Slashdot)，不信(例：Epinions)，ブロック・友だち解除(例：FacebookやTwitter)などのネガティブなエッジを作ることもできる．
...
次に，まずバランスパスとアンバランスパスを導入し，その後，符号付きグラフ用のグラフフィルタの設計を説明する． 一般的に，バランス理論(Heider, 1946; Cartwright and Harary, 1956)は，「自分の友人の友人は，自分の友人でもあるべき」，「自分の友人の敵は，自分の敵であるべき」という基本原則を提唱している． したがって，グラフ内の閉路 [^3] は「バランスが取れているか(**バランス状態**)」もしくは「バランスが取れていないか(**アンバランス状態**)」の2つの状態に分類されることになる．



![図8-A. (a) バランス状態，(b) アンバランス状態．](./fig/A8-1(a_b).png)
...
[^2]: 訳注：Epinionsは，かつて存在した消費者レビューサイトで，ユーザーが商品やサービスについての意見や評価を共有できるプラットフォームであった． これらの関係は正と負の両方のエッジを持つグラフとして表現することができる． SNSの人気が高まるにつれて，符号付きグラフはますます一般的になってきた． 符号付きグラフの正式な定義は2.6.4節で確認することができる．
[^3]: 訳注：グラフの閉路とは，グラフ内の特定の開始ノードから出発し，エッジを通じて一連の他のノードを通過し，最終的に同じ開始ノードに戻る経路である． 具体的には，負エッジの数が偶数個の閉路は**バランス状態**とみなされ，そうでなければ**アンバランス状態**とみなされる [^4] ．
[^4]: 訳注：例として，図8-Aにノード数が4個の閉路を示した（ノードをつなぐ実線が閉路である）．負のエッジが2個(偶数個)の場合（図8-A(a)），ノード $2$ とノード $4$ をつなぐ(破線)エッジは，バランス理論より，負エッジであると一意に決まるので，バランス状態である．一方，負エッジが1個(奇数個)の場合（図8-A(b)），ノード $2$ とノード $4$ をつなぐ(破線)エッジは一意に定まらない．これはバランス理論の原則を満たさないことを示しており，アンバランス状態であるとみなされる．
