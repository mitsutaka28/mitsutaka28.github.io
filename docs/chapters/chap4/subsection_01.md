...
グラフ埋め込み(Graph
Embedding)の目的は，与えられたグラフの各ノードを，「元のグラフで各ノードが持つ重要な情報を保持するような」低次元ベクトル表現に写像することである(これは一般にノード埋め込みとして知られている)．
...
その後，ヘテログラフや二部グラフ，多次元グラフ，符号付きグラフ，ハイパーグラフ，ダイナミックグラフに特化したグラフ埋め込みアルゴリズムを紹介する．
