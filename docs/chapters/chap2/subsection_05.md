...
現実世界で現れる多くのグラフでは，各ノードに特徴や属性が与えられている場合が多い．このようなグラフは，構造情報（ノード間の接続性）とデータ（ノードの属性）の両方を捉えた，「信号(シグナル)を持つグラフ」として見ることができる．信号付きのグラフは「グラフ $\symcal{G} = \left\\{\symcal{V},\symcal{E}\right\\}$ 」と，「ノードを実数値に写像する，グラフ領域で定義された写像 $f$ 」から構成される． 数学的には，この写像は以下のように表される．

 $$
 \nonumber
f\colon \symcal{V} \rightarrow \mathbb{R}^{N\times d}. $$
...
同様に，グラフ $\symcal{G}$ 上のグラフ信号 $\symbf{f}$ の"グラフフーリエ変換"は，以下のように表すことができる．
...
以上の議論より，すべての固有ベクトルは，グラフ $\symcal{G}$ のグラフフーリエ基底であり，対応する固有値はその周波数となることがわかる．グラフフーリエ変換は，式(2.12)に示すように，入力信号 $\symbf{f}$ を周波数の異なるグラフフーリエ基底に分解する処理とみなすことができ，得られた係数 $\hat{\symbf{f}}$ は，対応するグラフフーリエ基底が入力信号に対してどれだけ寄与しているかを表す． 
<div class="eg">
...
<strong>例 2.34</strong>
 図2.5に，空間領域およびスペクトル領域におけるグラフ信号をそれぞれ示した． 図2.5(a)は空間領域のグラフ信号，図2.5(b)は同じグラフ信号をスペクトル領域で示したものである．図2.5(b)において， $x$ 軸はグラフフーリエ基底（固有ベクトル）に対応する周波数（固有値）， $y$ 軸はそれらに対応するグラフフーリエ係数(各基底がどの程度グラフ信号に寄与しているか)を表している． 
</div>
