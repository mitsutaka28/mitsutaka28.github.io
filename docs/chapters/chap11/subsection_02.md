...
画像で検出された物体について, 質問依存グラフ $\mathcal{G}\_I$ が得られた後, 5.3.2節で導入したMoフィルターを用いてノード表現を生成する. ノード $v_i$ についての処理は次のように定式化される:
...
いずれのグラフもグラフニューラルネットワークで処理され, ノード表現を生成する. そしてそれらを統合して質問と画像のペア $(q, I)$ の表現を生成する. (Teney et al., 2017)では, 5.3.2節で紹介したGGNNフィルターを少し修正したものを使ってこれら2つのグラフを処理している. 修正したGGNNフィルターは次のように書くことができる:
